...
if (isset($_GET['unsuscribe'])) {
	...
	<p>Has anulado correctamente la inscripción de " . $_GET['unsuscribe'] . ".</p>
</div>";
}
?>
...
<div id="principal" class="w3-margin">
	<div class="w3-blue">
		<h2 class="w3-text-shadow w3-margin">Lista actividades inscrito <img class="w3-text-white"
			...
			foreach ($actividades->result_array() as $row) { ?>
				<div id='a<?php echo $row['id']; ?>' class="resalte w3-animate-opacity">
					...
					<span onclick="this.parentElement.parentElement.style.display='none'"
						...
						<p class="w3-margin-0 w3-yellow w3-padding"><b><i class="fa fa-exclamation-triangle"
																	   aria-hidden="true"></i>
								Estas seguro que quieres anular la inscripción de la actividad  <?php echo $row['nombre']; ?>?</b></p>
						<div class="w3-center">
							<a class="w3-margin w3-btn w3-green"
							   href="<?php echo base_url(); ?>user/unsuscribe?id=<?php echo $row['id']; ?>&nombre=<?php echo $row['nombre']; ?>">
								Si
							</a>
