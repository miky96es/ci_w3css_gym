...
if (isset($_GET['suscribe'])) {
	...
	<p>Te has inscrito correctamente  a " . $_GET['suscribe'] . ".</p>
</div>";
}
?>
<nav class="w3-sidenav w3-hide-large w3-blue w3-animate-left w3-card-2" style="z-index: 3; width: 250px; display: none;"
	...
	<a href="<?php echo base_url() ?>user"><b><i
...
<header class="w3-blue w3-hide-small w3-hide-medium w3-bar">
	<a class="w3-bar-item w3-button w3-hover-indigo" href="<?php echo base_url() ?>user"><b><i class="fa fa-home"
																							   aria-hidden="true"></i>
...
<div id="principal" class="w3-margin">
	<div class="w3-blue">
		<h2 class="w3-text-shadow w3-margin">Inscripción actividades <img class="w3-text-white"
			...
			foreach ($actividades->result_array() as $row) { ?>
				<div id='a<?php echo $row['id']; ?>' class="resalte w3-animate-opacity">
					...
					<span onclick="this.parentElement.parentElement.style.display='none'"
						...
						<p class="w3-margin-0 w3-green w3-padding"><b><i class="fa fa-exclamation-triangle"
																		  aria-hidden="true"></i>
								Estas seguro que quieres inscribirte en la actividad  <?php echo $row['nombre']; ?>?</b></p>
						<div class="w3-center">
							<a class="w3-margin w3-btn w3-green"
							   href="<?php echo base_url(); ?>user/suscribe?id=<?php echo $row['id']; ?>&nombre=<?php echo $row['nombre']; ?>">
								Si
							</a>
				...
				<tr>
					...
					<td>
						<button class="w3-btn w3-green w3-hover-pale-green"
